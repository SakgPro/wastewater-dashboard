...
def load_data():
    try:
        response = requests.get("https://wastewater-api-ymfn.onrender.com/api/download_data", timeout=10)
...
if not df.empty:
    ...
    selected_loc = st.sidebar.selectbox("Select Location", ["All Campus"])
...
if df.empty:
    st.warning("Awaiting telemetry from the sensor network API... Ensure the backend is active.")
else:
